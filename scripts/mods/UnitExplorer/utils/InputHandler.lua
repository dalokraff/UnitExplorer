...
function mod.handle_inputs()
    if Keyboard.pressed(Keyboard.button_index("f2")) then
        ...
        if mod.unit_explorer then
            local outline_system =
                Managers.state.entity:system("outline_system")
            ...
            local first_person_extension =
                ScriptUnit.extension(player_unit, "first_person_system")
            ...
            local hits = physics_world:immediate_raycast(camera_position,
                                                         camera_forward,
                                                         distance, "all",
                                                         "collision_filter",
            ...
            elseif closest_unit_hit == nil then
                ...
                if mod.outlined_unit then
                    outline_system:outline_unit(mod.outlined_unit, flag,
                                                channel, not do_outline,
                                                apply_method)
                end
                outline_system:outline_unit(closest_unit_hit, flag, channel,
                                            do_outline, apply_method)
    ...
    if Keyboard.pressed(Keyboard.button_index("f3")) then
        if mod.level_explorer then mod.level_explorer:toggle() end
    end
